...
class HrOvertimeMultiple(models.Model):
    ...
    def unlink(self):
        ...
        return super(HrOvertimeMultiple, self).unlink()
    ...
    employee_ids = fields.Many2many('hr.employee', 'employee_overtime_multiple_rel','employee_id','overtime_id' ,string="Employees", required=True,states={'validate':[('readonly', True)]})
    ...
    approve_dept_manager_id = fields.Many2one('res.users', string='Dep. Manager', readonly=True, copy=False)
    department_manager_id = fields.Many2one('hr.employee', string='Dep. Manager(RR)')
