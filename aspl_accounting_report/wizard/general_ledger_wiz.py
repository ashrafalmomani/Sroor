...
class account_general_ledger_wiz(models.TransientModel):
    ...
    def print_pdf(self, data):
        ...
        data['form'] = self.read(['date_from', 'date_to', 'journal_ids', 'display_account', 'target_move', 'include_init_balance', 'sortby'])[0]
        datas = {
            'ids': self._ids,
            'docs': self._ids,
        ...
        return self.env.ref('aspl_accounting_report.report_general_ledger').report_action(self, data=datas)
...
#         alignment_right.horz = xlwt.Alignment.HORZ_LEFT
        ...
        worksheet.write_merge(6, 6, 0, 2, 'Sort by : ' + 'Date' if self.sortby == 'sort_date' else  'Sort by : ' + 'Journal & Partner')
        ...
        report_obj = self.env['report.aspl_accounting_report.general_ledger_template']
        data = {}
        data['ids'] = self.env.context.get('active_ids', [])
        data['model'] = self.env.context.get('active_model', 'ir.ui.menu')
        data['form'] = self.read(
        ...
        report_data = report_obj.with_context(active_model='account.general.ledger.wiz')._get_report_values(self, data=datas)
        i = 9
        for each in report_data['Accounts']:
            worksheet.write_merge(i, i, 0, 5,each['code'] + ' ' + each['name'], fontbold)
            worksheet.write_merge(i, i, 6, 6, formatLang(self.env,float(each['debit']),currency_obj=self.env.user.company_id.currency_id), fontbold)
            worksheet.write_merge(i, i, 7, 7, formatLang(self.env,float(each['credit']),currency_obj=self.env.user.company_id.currency_id), fontbold)
            worksheet.write_merge(i, i, 8, 8, formatLang(self.env,float(each['balance']),currency_obj=self.env.user.company_id.currency_id), fontbold)
            if self.include_init_balance:
                i = i + 1
                worksheet.write_merge(i, i, 4,5 , 'Initial Balance')
                worksheet.write_merge(i, i, 6, 6, formatLang(self.env,float(each['init_bal'][0]),currency_obj=self.env.user.company_id.currency_id))
                worksheet.write_merge(i, i, 7, 7, formatLang(self.env,float(each['init_bal'][1]),currency_obj=self.env.user.company_id.currency_id))
                worksheet.write_merge(i, i, 8, 8, formatLang(self.env,float(each['init_bal'][2]),currency_obj=self.env.user.company_id.currency_id))
            i = i + 1
            for each_line in each['move_lines']:
                worksheet.write_merge(i, i, 0, 0, str(each_line['ldate']))
                worksheet.write_merge(i, i, 1, 1, each_line['lcode'])
                worksheet.write_merge(i, i, 2, 2, each_line['partner_name'])
                worksheet.write_merge(i, i, 3, 3, each_line['lref'])
                worksheet.write_merge(i, i, 4, 4, each_line['move_name'])
                worksheet.write_merge(i, i, 5, 5, each_line['lname'])
                worksheet.write_merge(i, i, 6, 6, formatLang(self.env,float(each_line['debit']),currency_obj=self.env.user.company_id.currency_id), stylePC)
                worksheet.write_merge(i, i, 7, 7, formatLang(self.env,float(each_line['credit']),currency_obj=self.env.user.company_id.currency_id), stylePC)
                worksheet.write_merge(i, i, 8, 8, formatLang(self.env,float(each_line['balance']),currency_obj=self.env.user.company_id.currency_id), stylePC)
                i = i + 1
