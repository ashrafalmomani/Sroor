...
class AccountAgedReceivable(models.TransientModel):
    ...
    def generate_aged_receivable(self):
        ...
        return self.env.ref('aspl_accounting_report.report_aged_receivable').report_action(self, data=datas)

    @api.multi
    def generate_account_receivable_xls(self):
        report_obj = self.env['report.aspl_accounting_report.aged_receivable_template']
        styleP = xlwt.XFStyle()
        ...
        for each in accont_moveline:
            ...
            worksheet.write_merge(i, i, 6, 6, formatLang(self.env,float(each['1']),currency_obj=self.env.user.company_id.currency_id), stylePC)
