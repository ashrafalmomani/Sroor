...
class tax_rpeort_wiz(models.TransientModel):
    ...
    def print_pdf(self, data):
        ...
        return self.env.ref('aspl_accounting_report.report_tax').report_action(self, data=datas)

    def print_xls(self):
        report_obj = self.env['report.aspl_accounting_report.tax_report_template']
        data = {}
        data['ids'] = self.env.context.get('active_ids', [])
        data['model'] = self.env.context.get('active_model', 'ir.ui.menu')
        data['form'] = self.read(['date_from', 'date_to'])[0]
