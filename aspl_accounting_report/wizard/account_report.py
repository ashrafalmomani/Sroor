...
class AccountingReport(models.TransientModel):
    ...
    def _print_report(self, data):
        ...
        return self.env.ref('aspl_accounting_report.action_report_financial').report_action(self, data=data, config=False)

    def print_xls(self):
        report_obj = self.env['report.aspl_accounting_report.report_financial']
        data = {}
        data['form'] = \
        ...
        datas['date_from_cmp'] = str(self.date_from_cmp) if self.date_from_cmp else False
        datas['date_to'] = str(self.date_to) if self.date_to else False
        datas['date_to_cmp'] = str(self.date_to_cmp) if self.date_to_cmp else False
        datas['debit_credit'] = str(self.debit_credit) if self.debit_credit else False
        ...
        worksheet.write_merge(0, 2, 0, 3,
                              ...
                              style=styleBorder)
        worksheet.write_merge(3, 3, 0, 3,'Balance Sheet' if self._context.get('from_balance_sheet') else 'Profit and Loss' ,style=styleBorder)
        worksheet.write_merge(4, 4, 0, 3, 'Target Moves : ' + 'All Entries' if self.target_move == 'all' else 'Target Moves : All Posted Entries')

        if self.date_from and self.date_to:
            worksheet.write_merge(5, 5, 0, 3,'Date From : ' + str(self.date_from) + ' ' + 'Date To : '+ str(self.date_to))
        i = 8
        if self.debit_credit:
            ...
            for each in lines:
                ...
                if each.get('level') != 0:
                    worksheet.write_merge(i, i, 0, 0, each.get('name'), normal_style)
                    worksheet.write_merge(i, i, 1, 1, formatLang(self.env,float(each.get('debit')),currency_obj=self.env.user.company_id.currency_id), fontbold)
                    worksheet.write_merge(i, i, 2, 2, formatLang(self.env,float(each.get('credit')),currency_obj=self.env.user.company_id.currency_id), fontbold)
                    worksheet.write_merge(i, i, 3, 3, formatLang(self.env,float(each.get('balance')),currency_obj=self.env.user.company_id.currency_id), fontbold)
                    i = i + 1
        if not self.enable_filter and not self.debit_credit:
            ...
            for each in lines:
                ...
                if each.get('level') != 0:
                    worksheet.write_merge(i, i, 0, 0, each.get('name'),normal_style)
                    worksheet.write_merge(i, i, 1, 1, formatLang(self.env,float(each.get('balance')),currency_obj=self.env.user.company_id.currency_id), fontbold)
                    i = i + 1
        if self.enable_filter and not self.debit_credit:
            ...
            for each in lines:
                if each.get('level') != 0:
                    ...
                    worksheet.write_merge(i, i, 0, 0, each.get('name'), normal_style)
                    worksheet.write_merge(i, i, 1, 1, formatLang(self.env,float(each.get('balance')),currency_obj=self.env.user.company_id.currency_id), fontbold)
                    worksheet.write_merge(i, i, 2, 2, formatLang(self.env,float(each.get('balance_cmp')),currency_obj=self.env.user.company_id.currency_id), fontbold)
                    i = i + 1
