...
class aged_receivable_template(models.AbstractModel):
    _name = 'report.aspl_accounting_report.aged_receivable_template'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        accont_moveline, total, dummy = self._get_partner_move_lines_custom(account_type, date_from, target_move, period_length)
        self.model = self.env.context.get('active_model')
        res_period = self.get_time_interval(date_from,period_length)
        data['form']['get_partner_lines'] = accont_moveline
        data['form']['get_total'] = total
        data['form']['periods'] = res_period
        return {
            'doc_ids': self.ids,
            'doc_model': model,
            'data': data['form'],
